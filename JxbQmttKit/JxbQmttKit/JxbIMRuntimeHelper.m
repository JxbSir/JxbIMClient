...
@implementation JxbIMRuntimeHelper
...
+ (NSArray *)propertyNames:(Class)klass {
    ...
    NSArray* arr = [JxbIMRuntimeHelper propertyNames:class_getSuperclass(klass)];
...
+ (Class)propertyClassForPropertyName:(NSString *)propertyName ofClass:(Class)klass {
	...
	for (unsigned int i = 0; i < propertyCount; ++i) {
		...
		if (strcmp(cPropertyName, name) == 0) {
			free(properties);
            const char* t = property_getTypeName(property);
            if (t) {
                NSString *className = [NSString stringWithUTF8String:t];
